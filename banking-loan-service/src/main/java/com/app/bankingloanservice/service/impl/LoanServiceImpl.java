...
public class LoanServiceImpl implements LoanService {
  ...
  public LoanResponse createLoanFromApplicationId(Long loanApplicationId) {
    ...
    LocalDate maturityDate = calculateMaturityDate(loanApplication.getDesiredDisbursementDate(),
        loanApplication.getDesiredLoanTermMonths());
    Loan newLoan = loanMapper.loanApplicationToLoan(loanApplication, generateUniqueLoanContractNo(),
        maturityDate);
    ...
    try {
      ...
      LoanInterestRateRequest loanInterestRateRequest = loanTypeMapper.toLoanInterestRateCreateDto(
          loanApplication.getLoanType());
      loanInterestRateRequest.setEffectiveFrom(
          newLoan.getDisbursementDate()); // Set the effective date

      // Save Loan Interest Rate using LoanInterestRateService
      LoanInterestRate loanInterestRate = loanInterestRateService.createLoanInterestRate(
          newLoan.getLoanId(), loanInterestRateRequest);
      ...
      String errorMessage = String.format(
          "Error occurred while creating loan from application with ID: %s",
          loanApplication.getLoanApplicationId());
      log.error(errorMessage, e); // Log the error and original exception
      throw new LoanCreationException(errorMessage,
          e); // Throw custom exception with message and cause
    }
  }
  ...
  public LoanResponse createLoan(LoanRequest loanRequest) {
    ...
    LocalDate maturityDate = calculateMaturityDate(loanRequest.getDisbursementDate(),
        loanRequest.getLoanTermMonths());
    LoanApplication loanApplication = loanApplicationService.getEntityById(
        loanRequest.getLoanApplicationId());
    LoanType loanType = loanTypeService.getLoanTypeById(loanRequest.getLoanTypeId());
    Loan newLoan = loanMapper.toEntity(loanRequest, loanApplication, generateUniqueLoanContractNo(),
        maturityDate, loanType);
    ...
    try {
      ...
      // Assign LoanInterestRate to the current Loan
      LoanInterestRate loanInterestRate = loanInterestRateService.createLoanInterestRate(
          newLoan.getLoanId(), loanRequest.getCurrentInterestRate());
      newLoan.setCurrentInterestRate(loanInterestRate);
      ...
      // Log error with detailed information and throw LoanCreationException
      String errorMessage = String.format(
          "Error occurred while creating loan with application ID: %s",
          loanApplication.getLoanApplicationId());
      log.error(errorMessage, e); // Log the error and original exception
      throw new LoanCreationException(errorMessage,
          e); // Throw custom exception with message and cause
  ...
  public Loan getLoanEntityById(Long loanId) {
    ...
    return loanRepository.findById(loanId).orElseThrow(() -> {
      String errorMessage = String.format(
          "Loan with ID %d not found. Please check the ID and try again.", loanId);
  ...
  @Override
  public List<LoanResponse> filters(long accountId, Long loanTypeId,
  ...
  private String generateUniqueLoanContractNo() {
    ...
    throw new LoanCreationException(
        "Unable to generate unique loan contract number after " + maxAttempts + " attempts.");
  }
