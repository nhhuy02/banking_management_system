...
(
    ...
    max_loan_amount            DECIMAL(19, 0) NOT NULL CHECK (max_loan_amount >= 0),            -- Maximum loan amount (VND)
...
(
    ...
    monthly_income            DECIMAL(19, 0)                                                             NOT NULL CHECK (monthly_income >= 0),          -- Monthly income
    ...
    desired_loan_amount       DECIMAL(19, 0)                                                             NOT NULL CHECK (desired_loan_amount >= 0),     -- Desired loan amount (VND)
...
(
    ...
    collateral_value       DECIMAL(19, 0) CHECK (collateral_value >= 0),                       -- Collateral value (VND)
...
(
    ...
    loan_amount                  DECIMAL(19, 0)                                  NOT NULL CHECK (loan_amount >= 0),                 -- Loan amount (VND)
    ...
    remaining_balance            DECIMAL(19, 0)                                  NOT NULL CHECK (remaining_balance >= 0),           -- Remaining loan balance (VND)
    total_paid_amount            DECIMAL(19, 0)                                  NOT NULL DEFAULT 0 CHECK (total_paid_amount >= 0), -- Total amount paid (VND)
...
(
    ...
    principal_amount             DECIMAL(19, 0)                      NOT NULL CHECK (principal_amount >= 0),                       -- Principal amount to be repaid
    interest_amount              DECIMAL(19, 0)                      NOT NULL CHECK (interest_amount >= 0),                        -- Interest amount to be repaid
    late_payment_interest_amount DECIMAL(19, 0)                      NOT NULL DEFAULT 0 CHECK (late_payment_interest_amount >= 0), -- Late payment interest amount
    total_amount                 DECIMAL(19, 0)                      NOT NULL CHECK (total_amount >= 0),                           -- Total amount to be repaid
    ...
    transaction_reference        VARCHAR(255),                                                                                     -- The reference ID for the transaction related to the repayment.
...
(
    ...
    past_due_interest_amount     DECIMAL(19, 0)                               NOT NULL DEFAULT 0 CHECK (past_due_interest_amount >= 0),     -- Past-due interest amount
    late_payment_interest_amount DECIMAL(19, 0)                               NOT NULL DEFAULT 0 CHECK (late_payment_interest_amount >= 0), -- Late payment interest amount
    prepayment_penalty_amount    DECIMAL(19, 0)                               NOT NULL DEFAULT 0 CHECK (prepayment_penalty_amount >= 0),    -- Prepayment penalty amount
    settlement_amount            DECIMAL(19, 0)                               NOT NULL CHECK (settlement_amount >= 0),                      -- Total settlement amount
